    ...
    <div>
        ...
        <div class="lg:p-8 ml-[30%] mt-12" style="overflow-x:auto;">
            <table class="border-collapse  border-2 border-gray-300">
                ...
                <th class="border border-gray-300 text-gray-700 font-sans text-base">First Name</th>
                ...
                <th class="border border-gray-300 text-gray-700 font-sans text-base">Message</th>

                </tr>
                <tr v-for="contact in contact.data" :key="contact.id">
                    <td >{{ contact.id }}</td>
                    <td class="border border-gray-300 text-gray-700 font-sans text-sm">{{ contact.firstname }}</td>
                    <td class="border border-gray-300 text-gray-700 font-sans text-sm">{{ contact.phone }}</td>
                    <td class="border border-gray-300 text-gray-700 font-sans text-sm">{{ contact.email }}</td>
                    <td class="border border-gray-300 text-gray-700 font-sans text-sm">{{ contact.massage }}</td>
                    ...
                    <td class="border border-gray-300">
                        <button type="button" @click="destroy(contact.id)" class="py-2 px-3 font-medium text-white font-sans text-base bg-red-600 rounded-md">
                            Delete
                        </button>
                    </td>
                </tr>
...
import { Inertia } from '@inertiajs/inertia';
    ...
    export default {
        ...
        setup() {
            ...
            function destroy(id) {
                Inertia.delete(`admin-message/${id}/destroy`, {
                    onBefore: () => confirm('Are you sure you want to delete this message'),
                    onSuccess: () => alert('message Successfully deleted'),
                })
            }
